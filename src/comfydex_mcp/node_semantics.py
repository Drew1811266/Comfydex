...
_REGISTRY: dict[str, NodeSemanticEntry] = {
    ...
    "LoraLoader": _entry(
        "LoraLoader",
        display_names=("Load LoRA", "Lora Loader"),
        category="Adapter",
        purpose="Apply a LoRA model to checkpoint model and CLIP outputs.",
        inputs=(
            _port("model", "Base diffusion model.", "MODEL"),
            _port("clip", "Base CLIP encoder.", "CLIP"),
            _port("lora_name", "LoRA filename selected from installed LoRA models.", "COMBO"),
            _port("strength_model", "Model strength for LoRA application.", "FLOAT"),
            _port("strength_clip", "CLIP strength for LoRA application.", "FLOAT"),
        ),
        outputs=(
            _port("model", "Model with LoRA influence applied.", "MODEL"),
            _port("clip", "CLIP with LoRA influence applied.", "CLIP"),
        ),
        parameter_strategies=("Default LoRA strengths to moderate values until a recipe overrides them.",),
        compatible_upstream=("CheckpointLoaderSimple",),
        compatible_downstream=("CLIPTextEncode", "KSampler"),
        failure_modes=("Missing or incompatible LoRA files can fail loading or distort output.",),
        repair_hints=("Confirm the LoRA file exists and lower strengths when results are unstable.",),
        requires_external_models=True,
    ),
    "ControlNetLoader": _entry(
        "ControlNetLoader",
        display_names=("Load ControlNet Model", "ControlNet Loader"),
        category="Control",
        purpose="Load a ControlNet model used to guide sampling from a control image.",
        inputs=(_port("control_net_name", "ControlNet filename selected from installed models.", "COMBO"),),
        outputs=(_port("control_net", "Loaded ControlNet model.", "CONTROL_NET"),),
        parameter_strategies=("Pick a ControlNet model that matches the preprocessor or control image type.",),
        compatible_upstream=(),
        compatible_downstream=("ControlNetApply", "ControlNetApplyAdvanced"),
        failure_modes=("Wrong ControlNet model type can produce weak or broken guidance.",),
        repair_hints=("Match pose, depth, lineart, or canny models to the control input.",),
        requires_external_models=True,
    ),
    "ControlNetApply": _entry(
        "ControlNetApply",
        display_names=("Apply ControlNet",),
        category="Control",
        purpose="Apply ControlNet guidance to positive and negative conditioning.",
        inputs=(
            _port("conditioning", "Conditioning to guide.", "CONDITIONING"),
            _port("control_net", "Loaded ControlNet model.", "CONTROL_NET"),
            _port("image", "Control image.", "IMAGE"),
            _port("strength", "Control strength.", "FLOAT"),
        ),
        outputs=(_port("conditioning", "Conditioning with ControlNet guidance.", "CONDITIONING"),),
        parameter_strategies=("Use moderate strength and expose it as a user-adjustable parameter.",),
        compatible_upstream=("CLIPTextEncode", "ControlNetLoader", "LoadImage"),
        compatible_downstream=("KSampler",),
        failure_modes=("Control image and model mismatch can cause poor structure adherence.",),
        repair_hints=("Use a control image and ControlNet model from the same control family.",),
    ),
    "ControlNetApplyAdvanced": _entry(
        "ControlNetApplyAdvanced",
        display_names=("Apply ControlNet Advanced",),
        category="Control",
        purpose="Apply ControlNet guidance with explicit start and end percent controls.",
        inputs=(
            _port("positive", "Positive conditioning.", "CONDITIONING"),
            _port("negative", "Negative conditioning.", "CONDITIONING"),
            _port("control_net", "Loaded ControlNet model.", "CONTROL_NET"),
            _port("image", "Control image.", "IMAGE"),
            _port("strength", "Control strength.", "FLOAT"),
            _port("start_percent", "Guidance start percent.", "FLOAT"),
            _port("end_percent", "Guidance end percent.", "FLOAT"),
        ),
        outputs=(
            _port("positive", "Guided positive conditioning.", "CONDITIONING"),
            _port("negative", "Guided negative conditioning.", "CONDITIONING"),
        ),
        parameter_strategies=("Keep start at 0 and end at 1 for simple recipes.",),
        compatible_upstream=("CLIPTextEncode", "ControlNetLoader", "LoadImage"),
        compatible_downstream=("KSampler",),
        failure_modes=("Overly high strength can overpower the prompt.",),
        repair_hints=("Lower strength or narrow the active percent range.",),
    ),
    "UpscaleModelLoader": _entry(
        "UpscaleModelLoader",
        display_names=("Load Upscale Model",),
        category="Model",
        purpose="Load an image upscale model.",
        inputs=(_port("model_name", "Upscale model filename.", "COMBO"),),
        outputs=(_port("upscale_model", "Loaded upscale model.", "UPSCALE_MODEL"),),
        parameter_strategies=("Use an installed upscaler appropriate for the image style.",),
        compatible_upstream=(),
        compatible_downstream=("ImageUpscaleWithModel",),
        failure_modes=("Missing upscale model prevents image upscaling.",),
        repair_hints=("Ask the user to pick an installed upscale model.",),
        requires_external_models=True,
    ),
    "ImageUpscaleWithModel": _entry(
        "ImageUpscaleWithModel",
        display_names=("Image Upscale With Model",),
        category="Image",
        purpose="Upscale an image with a loaded upscale model.",
        inputs=(
            _port("upscale_model", "Loaded upscale model.", "UPSCALE_MODEL"),
            _port("image", "Image to upscale.", "IMAGE"),
        ),
        outputs=(_port("image", "Upscaled image.", "IMAGE"),),
        parameter_strategies=("Use after loading a compatible upscaler and before SaveImage.",),
        compatible_upstream=("UpscaleModelLoader", "LoadImage", "VAEDecode"),
        compatible_downstream=("SaveImage", "PreviewImage"),
        failure_modes=("Large input images can exceed memory during upscaling.",),
        repair_hints=("Scale down first or choose a lighter upscale path.",),
        requires_external_models=True,
    ),
    "VAELoader": _entry(
        "VAELoader",
        display_names=("Load VAE",),
        category="Model",
        purpose="Load a standalone VAE for encoding or decoding.",
        inputs=(_port("vae_name", "VAE filename selected from installed VAE models.", "COMBO"),),
        outputs=(_port("vae", "Loaded VAE.", "VAE"),),
        parameter_strategies=("Use only when the checkpoint VAE is missing or a recipe requires a specific VAE.",),
        compatible_upstream=(),
        compatible_downstream=("VAEDecode", "VAEEncode", "VAEEncodeForInpaint"),
        failure_modes=("Wrong VAE can shift color or decode incorrectly.",),
        repair_hints=("Prefer the checkpoint VAE unless a known compatible VAE is requested.",),
        requires_external_models=True,
    ),
    "IPAdapterUnifiedLoader": _entry(
        "IPAdapterUnifiedLoader",
        display_names=("IPAdapter Unified Loader",),
        category="Adapter",
        purpose="Load IP-Adapter assets from ComfyUI_IPAdapter_plus for reference-image conditioning.",
        inputs=(
            _port("model", "Base diffusion model.", "MODEL"),
            _port("preset", "IP-Adapter preset.", "COMBO", required=False),
        ),
        outputs=(_port("ipadapter", "Loaded IP-Adapter handle.", "IPADAPTER"),),
        parameter_strategies=("Treat as explanatory support until capability resolution verifies the installed package.",),
        compatible_upstream=("CheckpointLoaderSimple",),
        compatible_downstream=("IPAdapterAdvanced",),
        failure_modes=("Package or model version drift can change inputs and break workflows.",),
        repair_hints=("Validate the installed node metadata before generating IP-Adapter workflows.",),
        requires_external_models=True,
        custom_node_package="ComfyUI_IPAdapter_plus",
        first_class=False,
    ),
    "IPAdapterAdvanced": _entry(
        "IPAdapterAdvanced",
        display_names=("IPAdapter Advanced",),
        category="Adapter",
        purpose="Apply IP-Adapter reference-image conditioning through ComfyUI_IPAdapter_plus.",
        inputs=(
            _port("model", "Base diffusion model.", "MODEL"),
            _port("ipadapter", "Loaded IP-Adapter handle.", "IPADAPTER"),
            _port("image", "Reference image.", "IMAGE"),
            _port("weight", "Reference influence weight.", "FLOAT"),
        ),
        outputs=(_port("model", "Model with IP-Adapter conditioning.", "MODEL"),),
        parameter_strategies=("Keep disabled for first-class generation until package metadata is validated.",),
        compatible_upstream=("IPAdapterUnifiedLoader", "LoadImage", "CheckpointLoaderSimple"),
        compatible_downstream=("KSampler",),
        failure_modes=("Installed IP-Adapter package versions can expose different node contracts.",),
        repair_hints=("Use object_info validation before building IP-Adapter graphs.",),
        requires_external_models=True,
        custom_node_package="ComfyUI_IPAdapter_plus",
        first_class=False,
    ),
